...
use App\User;
//use App\Http\Controllers\LA\Magisters;
...
class EmployeesController extends Controller
{
	...
	public $listing_cols = ['id', 'name', 'designation'];
	...
	public function show($id)
	{
		if(Module::hasAccess("Employees", "view")) {
			...
			if(isset($employee->id)) {
				...
				$magisters = DB::table('magisters')
					->select('magisters.id as id', 'prop_recenzent.name as prop_recenzent', 'recenzent.name as recenzent', 'student.name as student', 'magisters.subject as subject', 'magisters.status as status')
					->leftJoin('employees as prop_recenzent', 'magisters.prop_recenzent', '=', 'prop_recenzent.id')
					->leftJoin('employees as recenzent', 'magisters.recenzent', '=', 'recenzent.id')
					->leftJoin('employees as student', 'magisters.student', '=', 'student.id')
					->where('promotor', '=', $id)->get();
				$licencjats = DB::table('licencjats')
					->select('licencjats.id as id', 'prop_recenzent.name as prop_recenzent', 'recenzent.name as recenzent', 'student.name as student', 'licencjats.subject as subject', 'licencjats.status as status')
					->leftJoin('employees as prop_recenzent', 'licencjats.prop_recenzent', '=', 'prop_recenzent.id')
					->leftJoin('employees as recenzent', 'licencjats.recenzent', '=', 'recenzent.id')
					->leftJoin('employees as student', 'licencjats.student', '=', 'student.id')
					->where('promotor', '=', $id)->get();
	...
	public function change_password($id, Request $request) {
		...
		\Session::flash('success_message', 'Hasło zostało zmienione');
		...
		if(env('MAIL_USERNAME') != null && env('MAIL_USERNAME') != "null" && env('MAIL_USERNAME') != "") {
			...
			Log::info("Hasło użytkownika zostało zmienione: użytkownik: ".$user->email." Hasło: ".$request->password);
		}
